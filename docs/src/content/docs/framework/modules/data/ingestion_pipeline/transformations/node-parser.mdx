...
Node parsers are a simple abstraction that take a list of `Document` objects, and chunk them into `Node` objects, such that each node is a specific chunk of the parent document. When a document is broken into nodes, all of it's attributes are inherited to the children nodes (i.e. `metadata`, text and metadata templates, etc.). You can read more about `Node` and `Document` properties [here](/framework/modules/data).
...
The `SentenceSplitter` is the default `NodeParser` in vectorstores. It will split the text from a `Document` into sentences.
...
<Tabs items={["with reader", "with node:fs"]}>
	...
	import { MarkdownNodeParser } from "@vectorstores/core";
	...
	import { MarkdownNodeParser, Document } from "@vectorstores/core";
...
Try it out ⬇️
  ...
  <summary>Use it in browser</summary>
  		...
  		And also update the `next.config.js` to make `@vectorstores/env` work properly.
...
- [SentenceSplitter](/framework-api-reference/classes/sentencesplitter/)
- [MarkdownNodeParser](/framework-api-reference/classes/markdownnodeparser/)
- [CodeSplitter](/framework-api-reference/classes/codesplitter/)
