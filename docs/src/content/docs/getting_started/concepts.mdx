...
Under the hood, vectorstores splits your documents into smaller chunks called `Node`s, calculates vector embeddings for each node, and stores them in memory or to disk. Each node maintains a reference to its source document. This chunking enables more precise retrieval compared to storing entire documents.
...
vectorstores provides composable modules that help you build and integrate RAG pipelines for Q&A, chatbot, or as part of an agent.
